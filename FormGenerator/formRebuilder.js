...
const FormGenerator = () => {
  ...
  const submitData = (submissionJson, status, partialSave) => {
  ...
  const onStart = () => {
    const config = {
      method: 'get',
      url: `${process.env.REACT_APP_BACKEND_API}/form/?_id=${formId}`,
      headers: {
        'Content-Type': 'application/json',
      },
    };
    ...
    axios(config)
      .then((data) => {
        ...
        splitPages(data.data.data.formJson);
      })
      .catch((error) => {
        console.log(error);
      });
